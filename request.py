...
if not api_key:
    raise EnvironmentError("La clé API OpenAI n'est pas définie. Veuillez définir la variable d'environnement 'API_KEY'.")
...
def send_to_gpt(prompt, api_key):
    ...
    try:
        ...
        print(f"Erreur lors de l'envoi de la requête à OpenAI : {e}")
        return None
...
def main(cv_test_file, save_path):
    ...
    for compt, titre in enumerate(tab_titres):
        ...
        if sorted_info is not None:
            ...
            print("Erreur lors de la récupération des données auprès de OpenAI.")
